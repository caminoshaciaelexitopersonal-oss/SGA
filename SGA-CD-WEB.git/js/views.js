    ...
    contentArea.addEventListener('click', async (e) => {
        ...
        if (e.target.id === 'btn-crear-area') {
            try {
                ...
                const form = document.getElementById('crear-area-form');
                form.addEventListener('submit', (submitEvent) => {
                    ...
                    handleFormSubmit({
                        form,
                        feedbackDiv: document.getElementById('modal-feedback'),
                        token,
                        endpoint: `${config.apiBaseUrl}/api/v1/areas`,
                        method: 'POST',
                        body: newArea,
                        validationFn: validateAreaForm,
                        successCallback: () => renderContentForView('gestionar-áreas', token, 'admin_empresa')
                    });
                });
            } catch (error) {
                openModal('Error', `<p class="message-error">${error.message}</p>`);
        ...
        if (e.target.classList.contains('btn-editar-area')) {
            ...
            try {
                ...
                const form = document.getElementById('editar-area-form');
                form.addEventListener('submit', (submitEvent) => {
                    submitEvent.preventDefault();
                    const updatedArea = {
                        nombre: document.getElementById('area-nombre-input').value,
                    ...
                    handleFormSubmit({
                        ...
                        feedbackDiv: document.getElementById('modal-feedback'),
                        token,
                        endpoint: `${config.apiBaseUrl}/api/v1/areas/${areaId}`,
                        method: 'PUT',
                        body: updatedArea,
                        validationFn: validateAreaForm,
                        successCallback: () => renderContentForView('gestionar-áreas', token, 'admin_empresa')
...
async function handleFormSubmit({ form, feedbackDiv, token, endpoint, method, body, successCallback, validationFn }) {
    if (validationFn && !validationFn(form, feedbackDiv)) return;
    ...
    try {
        ...
        if (response.ok) {
            feedbackDiv.className = 'message-success';
            feedbackDiv.textContent = '¡Operación exitosa!';
            setTimeout(() => {
                closeModal();
                successCallback();
            }, 1500);
...
function validateAreaForm(form, feedbackDiv) {
    let errors = [];
    const nombre = form.querySelector('#area-nombre-input');
    if (nombre && nombre.value.trim() === '') errors.push('El nombre del área no puede estar vacío.');
...
function validateForm(form, feedbackDiv) {
    ...
    const nombre = form.querySelector('#usuario-nombre-input');
    const email = form.querySelector('#usuario-email-input');
    const password = form.querySelector('#usuario-password-input');
    const rol = form.querySelector('#usuario-rol-select');

    if (nombre && nombre.value.trim() === '') errors.push('El nombre no puede estar vacío.');
    if (email && !emailRegex.test(email.value)) errors.push('El formato del email no es válido.');
    if (rol && rol.value === '') errors.push('Debe seleccionar un rol.');

    // La contraseña solo es obligatoria en el formulario de creación
            ...
            try {
                ...
                form.addEventListener('submit', (submitEvent) => {
                    ...
                    handleFormSubmit({
                        ...
                        feedbackDiv: document.getElementById('modal-feedback'),
                        token,
                        endpoint: `${config.apiBaseUrl}/api/v1/users`,
                        method: 'POST',
                        body: newUser,
                        validationFn: validateForm,
                        successCallback: () => renderContentForView('gestionar-usuarios', token, 'admin_general')
                    });
                });
                ...
                form.addEventListener('submit', (submitEvent) => {
                    ...
                    if (password) {
                        if (password.length < 6) {
                            const feedbackDiv = document.getElementById('modal-feedback');
                            feedbackDiv.className = 'message-error';
                            feedbackDiv.textContent = 'La nueva contraseña debe tener al menos 6 caracteres.';
                            return;
                        }
                    ...
                    handleFormSubmit({
                        ...
                        feedbackDiv: document.getElementById('modal-feedback'),
                        token,
                        endpoint: `${config.apiBaseUrl}/api/v1/users/${userId}`,
                        method: 'PUT',
