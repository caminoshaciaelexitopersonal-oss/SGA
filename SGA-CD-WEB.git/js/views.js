...
async function renderVerificarRolesView(token) {
    ...
    contentArea.innerHTML = `<div class="view-header"><h2><i class="fas fa-user-shield"></i> Verificar Roles en la BD</h2></div><table class="data-table"><thead><tr><th>ID</th><th>Nombre</th><th>Descripción</th></tr></thead><tbody>${rows || '<tr><td colspan="3">No se encontraron roles.</td></tr>'}</tbody></table><div class="actions-footer"><h3>Roles Faltantes</h3>${faltHtml}</div>`;
...
function abrirModalCrearRol(rolNombre, token) {
    const modalBodyContent = `<form id="crear-rol-form" style="display: flex; flex-direction: column; gap: 1rem;"><p>Creando el rol: <strong>${rolNombre}</strong></p><div class="form-group"><label for="rol-descripcion-input">Descripción</label><textarea id="rol-descripcion-input" class="form-textarea" required></textarea></div><div class="form-actions"><button type="submit" class="btn-primary">Confirmar</button></div></form><div id="modal-feedback"></div>`;
