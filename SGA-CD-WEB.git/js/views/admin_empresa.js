...
async function renderSuscripcionView(token) {
    ...
    // 4. Handle Mercado Pago payment submission
    ...
    mpBtn.addEventListener('click', async (e) => {
        ...
        try {
            const response = await fetch(`${config.apiBaseUrl}/api/v1/billing/mercadopago/create-preference`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                body: JSON.stringify({ amount: 10.00, currency: 'usd' }) // Example amount
            });
            ...
            window.location.href = redirect_url; // Redirect user to Mercado Pago
